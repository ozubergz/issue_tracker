  ...
  var id;
    ...
    suite('POST /api/issues/{project} => object with issue data', function() {
      ...
      test('Required fields filled in', function(done) {
        chai.request(server)
          ...
          .end(function(err, res) {
            ...
            assert.equal(res.body.issue_text, 'txt');
      ...
      test('Missing required fields', function(done) {
        chai.request(server)
          .post('/api/issues/test')
          .send({
            created_by: 'Function Test - Missing required fields'
          })
          .end(function(err, res) {
            assert.equal(res.status, 200);
    ...
    suite('PUT /api/issues/{project} => text', function() {
      ...
      test('Multiple fields to update', function(done) {
        chai.request(server)
          ...
          .send({
            _id: id,
            issue_title: 'updated issue title', issue_text: 'updated issue text'
          })
          .end(function(err, res) {
            assert.equal(res.status, 200);
            assert.equal(res.text, 'successfully updated');
    ...
    suite('GET /api/issues/{project} => Array of objects with issue data', function() {
      ...
      test('Multiple filters (test for multiple fields you know will be in the db for a return)', function(done) {
        chai.request(server)
          .get('/api/issues/test')
          .query({issue_text: 'txt'})
          .end(function(err, res) {
            assert.equal(res.status, 200);
            assert.equal(res.body[0].issue_text, 'txt');
            assert.property(res.body[0], 'issue_title');
            assert.property(res.body[0], 'issue_text');
            assert.property(res.body[0], 'created_on');
            assert.property(res.body[0], 'updated_on');
